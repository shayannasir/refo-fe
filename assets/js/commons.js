...
const INVALID_PASSWORD_MSG =
  "Password should contain at least one uppercase letter, one numeric digit, and one special character. Minimum length should be 8 and maximum 15";
...
class API_INIT {
  constructor() {
    ...
    this.getToken = function () {
      if (AUTH_TOKEN && AUTH_TOKEN.length > 100) return "Bearer " + AUTH_TOKEN;
      return false;
    };
  }
}
...
const tokenInCookie = function () {
  if (
    document.cookie
      .split(";")
      .some((item) => item.trim().startsWith(PREFIX + "authToken="))
  ) {
    var token = document.cookie
      .split("; ")
      .find((row) => row.startsWith(PREFIX + "authToken="))
      .split("=")[1];
    var user = document.cookie
      .split("; ")
      .find((row) => row.startsWith(PREFIX + "user="))
      .split("=")[1];
    var role = document.cookie
      .split("; ")
      .find((row) => row.startsWith(PREFIX + "role="))
      .split("=")[1];
